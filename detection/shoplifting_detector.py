...
class ShopliftingDetector:
    ...
    def __init__(self):
        ...
        self.shelf_interaction_threshold = 0.6
        self.concealment_threshold = 0.7
        self.shoplifting_threshold = 0.9
    ...
    def _analyze_shelf_interaction_behavior(self, person_data: Dict, shelf_zone: Dict) -> float:
        """
        Analyze specific behaviors that indicate shelf interaction
        """
        track_data = person_data['track_data']
        ...
        if movement_distances:
            ...
            # Low movement indicates stationary behavior (reaching for items)
            if avg_movement < 20:
                score += 0.4

        # Check for hand-reaching behavior (simplified - would need pose estimation)
        bbox_changes = self._analyze_bbox_changes(positions[-5:])
        if bbox_changes > 0.3:
            score += 0.3
    ...
    def _detect_item_taking_behavioral(self, person_data: Dict, shelf_interaction: Dict) -> Dict:
        """
        Behavioral detection of item taking (without object detection)
        """
        track_data = person_data['track_data']
        positions = track_data['positions']
        ...
        # Check for approach-stop-reach pattern
        if len(positions) >= 10:
            early_positions = positions[-10:-5]
            ...
            if early_movement > 30 and recent_movement < 15:
                score += 0.5

        # Check for bbox changes indicating reaching
        bbox_changes = self._analyze_bbox_changes(positions[-3:])
        if bbox_changes > 0.4:
            score += 0.4
